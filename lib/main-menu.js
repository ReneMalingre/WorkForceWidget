// This is the main application control
// with multi-level menus and inquirer prompts

// import statements
// some utilities to parse the data from the database
const { getIDFromNameWithID, getNameFromNameWithID } = require('./utils')

// database connection (used by all queries) :the database is connected to in index.js
// closeConnection() is called in main-menu.js to close the connection when the user exits the app
...
async function addEmployeePrompts () {
  ...
  return inquirer.prompt([
    ...
    .then(async answer => {
      ...
      const managerID = getIDFromNameWithID(managerAnswer)
      return await addEmployee(answer.firstName.trim(), answer.lastName.trim(), roleID, managerID).then(console.log(chalk.yellowBright(`\nEmployee ${answer.firstName.trim()} ${answer.lastName.trim()} added`)))
    })
}
...
async function addDepartmentPrompts () {
  ...
  return inquirer.prompt([
    ...
    .then(async answer => {
      return await addDepartment(answer.name.trim()).then(console.log(chalk.yellowBright(`\nDepartment (${answer.name.trim()}) added`)))
    })
}
...
async function addRolePrompts () {
  ...
  return inquirer.prompt([
    ...
    .then(async answer => {
      ...
      const departmentID = getIDFromNameWithID(departmentAnswer)
      return await addRole(answer.title.trim(), answer.salary, departmentID).then(console.log(chalk.yellowBright(`\nRole (${answer.title.trim()}) added`)))
    })
}
