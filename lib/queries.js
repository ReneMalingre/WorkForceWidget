// Runs the MySQL Queries from Main Menu
// and returns the results in a table where appropriate
...
async function addRole (roleTitle, roleSalary, departmentId) {
  const query = `INSERT INTO
    ...
    VALUES (?, ?, ?)`
  // remove any non-numeric characters from the salary
  roleSalary = roleSalary.replace(/[^\d.]/g, '')
  // format the salary to 2 decimal places as a number
  roleSalary = Number(roleSalary).toFixed(2)
